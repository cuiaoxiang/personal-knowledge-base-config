...
- **`sources/` (分布式信源核心)**：`sources` 不再仅仅是一个物理文件夹，而是一个**逻辑上的数据流网络**。
  - **本地 Inbox**：作为当前知识库的本地信源池，存放 Web Clipper 剪藏、临时会话的总结 (raw data) 以及所有本地缓存的附件（如 `sources/attachments/` 下的图片）。为了分类清晰，其下常设三个子目录：
...
- **核心政策**：自动处理 `sources/` 下的原始文件并提炼写入 `wiki/`。若指定特定文件，则直接录入该文件；若未指定，则增量扫描 `sources/chats/`、`sources/clippings/` 和 `sources/transcripts/` 下 `mtime` 晚于 `.last_sync_time` 的变更文件。同步完成后更新 `.last_sync_time` 记录状态作为逻辑归档，原文件永久保留在原目录中。
