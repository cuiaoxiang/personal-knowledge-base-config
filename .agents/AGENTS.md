...
4. **排除忽略文件的前置判断 (Ignore-first)**：在触发上述任何 Git 检查和提交动作前，必须首先检查变更文件是否均被 `.gitignore` 规则所忽略。若当前会话修改的所有文件均被忽略（工作区没有需要追踪的有效变更），应当直接跳过所有的 Git 基底状态检查及提交操作，避免冗余的命令行开销。**强制执行顺序**：必须先用 `grep` 或 `view_file` 读取 `.gitignore` 内容并完成忽略判断，确认有未被忽略的有效变更文件后，才允许发出任何 `git fetch` / `git status` / `git commit` 等命令。严禁先发出 git 命令、事后才去检查 `.gitignore`。
