...
- 对于提炼出的每个实体：
  - **增量合并 (Merge)**：如果该实体在 `wiki/` 下已存在同名 Markdown 页面（大小写不敏感），将新事实、操作规程或结论合并/追加到已有页面。
    ...
    - 将原信源以 `[[sources/chats/文件名]]` 或 `[[sources/clippings/文件名]]` 的双链形式追加到 YAML 的 `sources:` 列表中（若已存在则不重复添加）。
  ...
  - **全新创建 (New)**：如果实体尚不存在，创建 `wiki/实体名称.md`（优先使用中文作为文件名与标题）。
    - 统一包含如下 YAML 元数据模板：
      ...
      sources:
        - "[[sources/chats/原文件名]]"  # 或 "[[sources/clippings/原文件名]]"
      ---
      ```
...
- **原文件保留**：原文件永久保留在 `sources/chats/` 或 `sources/clippings/` 原位，不再进行任何物理移动或删除。
